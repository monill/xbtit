<?php

// USERDETAILS.PHP LANGUAGE FILE

$language['USERNAME']           = 'Kullanıcı Adı';
$language['EMAIL']              = 'e-Posta';
$language['LAST_IP']            = 'Son IP';
$language['USER_LEVEL']         = 'Seviye';
$language['USER_JOINED']        = 'Katılış Tarihi';
$language['USER_LASTACCESS']    = 'Son Erişim';
$language['PEER_COUNTRY']       = 'Ülke';
$language['USER_LOCAL_TIME']    = 'Kullanıcının Yerel Zamanı';
$language['DOWNLOADED']         = 'İndirilen';
$language['UPLOADED']           = 'Yüklenilen';
$language['RATIO']              = 'Oran';
$language['FORUM']              = 'Forum';
$language['POSTS']              = 'Mesaj';
$language['POSTS_PER_DAY']      = 'günde %s mesaj';
$language['TORRENTS']           = 'Torrentler';
$language['FILE']               = 'Dosya';
$language['ADDED']              = 'Eklenme';
$language['SIZE']               = 'Boyut';
$language['SHORT_S']            = 'S';
$language['SHORT_L']            = 'L';
$language['SHORT_C']            = 'T';
$language['NO_TORR_UP_USER']    = 'Kullanıcı henüz hiç torrent yüklemedi!';
$language['ACTIVE_TORRENT']     = 'Aktif Torrentler';
$language['PEER_STATUS']        = 'Durum';
$language['NO_ACTIVE_TORR']     = 'Aktif Torrent Yok';
$language['PEER_CLIENT']        = 'İstemci';
$language['EDIT']               = 'Düzenle';
$language['DELETE']             = 'Sil';
$language['PM']                 = 'ÖM';
$language['BACK']               = 'Geri';
$language['NO_HISTORY']         = 'Görüntülenecek geçmiş yok...';
$language['GUEST_DETAILS']      = 'Ziyaretçi hesabının detaylarını görüntüleyemezsiniz!';
<<<<<<< HEAD
=======
?>
>>>>>>> 862946f1c9960b40ff7f0dacd8283178fbbbab2b
