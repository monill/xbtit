<?php

// USERDETAILS.PHP LANGUAGE FILE

$language['USERNAME']           = 'Uživatelské jméno';
$language['EMAIL']              = 'Email';
$language['LAST_IP']            = 'Poslední Ip';
$language['USER_LEVEL']         = 'Hodost';
$language['USER_JOINED']        = 'Založeno';
$language['USER_LASTACCESS']    = 'Poslední přístup';
$language['PEER_COUNTRY']       = 'Země';
$language['USER_LOCAL_TIME']    = 'Lokální čas uživatele';
$language['DOWNLOADED']         = 'Downloaded';
$language['UPLOADED']           = 'Uploaded';
$language['RATIO']              = 'Ratio';
$language['FORUM']              = 'Fórum';
$language['POSTS']              = 'Příspěvky';
$language['POSTS_PER_DAY']      = '% příspěvků za den';
$language['TORRENTS']           = 'Torrenty';
$language['FILE']               = 'Soubor';
$language['ADDED']              = 'Přidáno';
$language['SIZE']               = 'Velikost';
$language['SHORT_S']            = 'S';
$language['SHORT_L']            = 'L';
$language['SHORT_C']            = 'C';
$language['NO_TORR_UP_USER']    = 'Uživatel prozatím nenehrál žádný torrent!';
$language['ACTIVE_TORRENT']     = 'Aktivní torrenty';
$language['PEER_STATUS']        = 'Status';
$language['NO_ACTIVE_TORR']     = 'Žádné aktivní torrenty';
$language['PEER_CLIENT']        = 'Klient';
$language['EDIT']               = 'Upravit';
$language['DELETE']             = 'Smazat';
$language['PM']                 = 'PM';
$language['BACK']               = 'Zpět';
$language['NO_HISTORY']         = 'Žádná historie k zobrazení...';
$language['GUEST_DETAILS']      = 'Nemůžete vidět detaily účtu hosta!';
<<<<<<< HEAD
=======
?>
>>>>>>> 862946f1c9960b40ff7f0dacd8283178fbbbab2b
